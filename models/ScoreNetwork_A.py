...
from models.attention import AttentionLayer, HAttentionLayer
...
#         # self.final = MLP(num_layers=3, input_dim=self.fdim, hidden_dim=2 * self.fdim, output_dim=1,
#         #                  use_bn=False, activate_func=F.elu)
#         self.final = MLP(num_layers=3, input_dim=self.fdim+1, hidden_dim=2 * self.fdim, output_dim=1,
#                          use_bn=False, activate_func=F.elu)
#         self.temb_net = MLP(num_layers=3, input_dim=self.nfeat, hidden_dim=2 * self.nfeat, output_dim=self.nfeat,
#                             use_bn=False, activate_func=F.elu)
#         # self.temb_net = MLP(num_layers=3, input_dim=self.fdim, hidden_dim=2 * self.fdim, output_dim=self.fdim,
#         #                     use_bn=False, activate_func=F.elu)
#
#         self.mask = torch.ones([self.max_node_num, self.max_node_num]) - torch.eye(self.max_node_num)
...
#         dist = self.manifold.dist(x[:,:,None,:],x[:,None,:,:],keepdim=True)
#         if self.manifold is not None:
#             x = exp_after_transp0(x, self.temb_net(temb), self.manifold)
#             x = self.centroid(x)
#
#         adjc = pow_tensor(adj, self.c_init)
#
...
#         out_shape = adjs.shape[:-1]  # B x N x N
#         # adjs = adjs+self.temb_net(temb).unsqueeze(1)  # (b,1,1,self.fdim)
#         # score = self.final(adjs).view(*out_shape)
#         score = self.final(torch.cat([adjs,dist],dim=-1)).view(*out_shape) * self.time_scale(temb)
#         self.mask = self.mask.to(score.device)
#         score = score * self.mask
#         score = mask_adjs(score, flags)
#         return score
...
#         temb = get_timestep_embedding(t, self.nfeat)
#         if self.manifold is not None:
#             x = exp_after_transp0(x, self.temb_net(temb), self.manifold)    # 或许不应该在节点加时间信息
#             x = self.centroid(x)
#
#         adjc = pow_tensor(adj, self.c_init)
#
